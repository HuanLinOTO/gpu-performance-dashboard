...
export function GPULeaderboard() {
  ...
  console.log("GPULeaderboard rendered with language:", language)
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            >
              {/* <img
                src="/favicon.svg"
                alt="GPU Leaderboard Logo"
                className="w-10 h-10"
              /> */}
              <div>
                <h1 className="text-3xl font-bold text-balance">{t.leaderboard.title}</h1>
            ...
            >
              ...
              <Badge variant="outline" className="text-xs">
                {data.length} {t.leaderboard.devices}
              </Badge>
              {lastUpdated && (
                <Badge variant="secondary" className="text-xs">
                  {t.leaderboard.updated} {lastUpdated.toLocaleTimeString()}
                </Badge>
              )}
            </motion.div>
          </div>
            ...
            <TabsContent value="table" className="space-y-6 animate-in fade-in-50 duration-300">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline">
                  {filteredData.length} {t.table.of} {data.length} {t.leaderboard.devices}
                </Badge>
              </div>
              <PerformanceTable
                data={filteredData}
